...
function EsprimaJSParser(esprima, logfunc){
    this.text = null;
...
mixin(EsprimaJSParser.prototype, 
    {
        parse:function(text){
            this.text = text;
            this._stack = [this.result];
            var now = new Date().getTime();
            var tree = this.esprima.parse(text, {range:true, loc: true});
            if(this.logfunc)
        ...
        _travelTree:function(tree){
            ...
            if(isFunction){
                var item = {name: tree.id? tree.id.name: null, line:  tree.loc.start.line, fullName: tree.longName,
                    start: tree.range[0], stop: tree.range[1]
                };
                this._addNode(item);
            }else if(type == 'AssignmentExpression'){
                var right = tree.right;
